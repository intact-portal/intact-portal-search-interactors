...
public class InteractorSearchService {
    ...
    public Map<String, Page<SearchInteractor>> resolveInteractorList(List<String> terms, boolean identifierSearch) {
        ...
        for (String term : terms) {
            results.put(term, resolveInteractor(term, identifierSearch));
        }

        return sortByTotalElements(results, false); //Descending
    }

    public Page<SearchInteractor> resolveInteractor(String query, boolean identifierSearch) {
        //TODO Paginate the results (for know it should cover disambiguation with 50 elements per page)
        if (identifierSearch) {
            return interactorRepository.resolveInteractorByIdsOrName(escapeQueryChars(query), PageRequest.of(0, 50));
        } else {
